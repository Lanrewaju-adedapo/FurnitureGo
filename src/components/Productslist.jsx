  ...
  return (
    ...
    <div className='mt-10 border-t-black py-7 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 px-4 sm:px-10 lg:px-20 gap-x-8 gap-y-10'>
      {Products.map((product) => (
        <div key={product.id} href='#' className='flex flex-col justify-center items-center pb-6 shadow-sm hover:shadow-xl hover:scale-105 transition-all duration-300 ease-in-out' 
      ...
      {
        productModal && (
          <Popup product={productModal} onClose={closePopup} />
        )
      }
